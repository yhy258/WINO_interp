...
class FourierBlock(nn.Module): 
    ...
    """
    def __init__(self, hidden_size, inner_expand, n_modes, num_blocks=8, sparsity_threshold=0.01, hard_thresholding_fraction=1, hidden_size_factor=1, act_func='GELU', in_ch_sf=False, weight_sharing=False, inner_nonlinear=False, bias=False):
        super().__init__()
    ...
    def _fourier_forward(self, x, dim=-2): # BS, C, H, W
        # similar to factorized FNO modules
        if dim == -2:
            ...
            if n_mode == x_ft.size(-2):  # full mode
                if self.weight_sharing:
                    out_ft = torch.einsum("bnixy,iox->bnoxy", x_ft, self.weight_1)
                else:
                    out_ft = torch.einsum("bnixy,niox->bnoxy", x_ft, self.weight_1)
                ...
                if self.weight_sharing:
                    out_ft[..., :n_mode, :] = torch.einsum("bnixy,iox->bnoxy", x_ft[..., :n_mode, :], self.weight_1)
                else:
                    out_ft[..., :n_mode, :] = torch.einsum("bnixy,niox->bnoxy", x_ft[..., :n_mode, :], self.weight_1)
                out_ft = out_ft.reshape(BS, -1, H, W)
            if self.in_ch_sf:
            ...
            if n_mode == x_ft.size(-1):
                if self.weight_sharing:
                    out_ft = torch.einsum("bnixy,ioy->bnoxy", x_ft, self.weight_2)
                else:    
                    out_ft = torch.einsum("bnixy,ioy->bnoxy", x_ft, self.weight_2)
            else:
                out_ft = self.get_zero_padding(
                ...
                if self.weight_sharing:
                    out_ft[..., :n_mode] = torch.einsum("bnixy,ioy->bnoxy", x_ft[..., :n_mode], self.weight_2)
                else:
                    out_ft[..., :n_mode] = torch.einsum("bnixy,nioy->bnoxy", x_ft[..., :n_mode], self.weight_2)
                out_ft = out_ft.reshape(BS, -1, H, W)
            if self.in_ch_sf:
                if self.inner_nonlinear:
                    out_ft = self.comp_act_func(out_ft)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        xx = self._fourier_forward(xx, dim=-1)
        xy = self._fourier_forward(xy, dim=-2)
...
class SpectralChannelBlock(nn.Module):
    def __init__(self, in_channels, inner_expand, expand_rate, act_func, mode, num_blocks=8, wp_addition=False, wp_mult=False, ch_process_block=True, pre_spec=False, batch_norm=False, sparsity_threshold=0.01, hard_thresholding_fraction=1, hidden_size_factor=1, dropout=0.0, drop_path_rate=0.0, fourier_bias=False, ffn_simplegate=False, in_ch_sf=False, weight_sharing=False, inner_nonlinear=False, depthwise_conv_module=False):
        super().__init__()
        self.fno = FourierBlock(in_channels, inner_expand, mode, num_blocks, sparsity_threshold, hard_thresholding_fraction, hidden_size_factor, in_ch_sf=in_ch_sf, weight_sharing=weight_sharing, inner_nonlinear=inner_nonlinear, bias=fourier_bias)
        self.wp_addition = wp_addition
        ...
        if ch_process_block:
            ...
            self.spec = nn.Identity()
            
        if self.wp_addition or self.wp_mult:
            self.wp_conv = nn.Conv2d(in_channels, in_channels, kernel_size=1)
            # self.wp_conv = nn.Linear(in_channels, in_channels)
            if self.wp_mult:
                self.post_conv = nn.Conv2d(in_channels, in_channels, kernel_size=1)
                # self.post_conv = nn.Linear(in_channels, in_channels)
        
        if batch_norm: # we do not use batch normalization. 
    ...
    def forward(self, x, emb=None):
        ...
        if self.pre_spec:
            ...
            x = self.spec(x)    
        if self.wp_addition:
            x = x + self.wp_conv(emb)
        if self.wp_mult:
            wp_emb = self.wp_conv(emb)
            x = self.post_conv(x * wp_emb)
            mult_x = x
...
class WINO(nn.Module):
    def __init__(
        ...
        wp_addition: bool = False,
        wp_mult: bool = False,
        wp_inputcat: bool = False,
        pre_spec: bool = True,
    ...
    def build_layers(self):
        ...
        features = [
            SpectralChannelBlock(
                ...
                mode=mode,
                num_blocks=hnb,
                ch_process_block=self.ch_process_block,
                wp_addition=self.wp_addition,
                wp_mult=self.wp_mult,
                pre_spec=self.pre_spec,
    ...
    def forward(self, x: Tensor, wvl: Tensor, dl: Tensor):
        ...
        x = self.stem(x)
        if self.wp_addition or self.wp_mult:
            cat = torch.cat([x, wpa_embed], dim=1)
            cat = F.pad(cat, [0, self.padding, 0, self.padding])
            x, wpa_embed = torch.chunk(cat, 2, dim=1)
        else:
